...
#############################################
    ...
    # 어제 뽑힌 학생은 오늘 룰렛 대상에서 제외하되, 전체 룰렛판에는 표시
    ...
    def draw_roulette(names, startangle=0, winner_idx=None, show_labels=True):
        ...
        # 화살표 표시기, paper 좌표계
        fig.add_shape(
            type="line", x0=0.5, y0=1.02, x1=0.5, y1=1.15,
    ...
    if today_hero:
        ...
        steps = 60  # 단계 수 증가로 부드러운 회전
        for i in range(steps):
            # cubic ease-out, 회전 끝부분이 자연스럽게 느려짐
            t = (i + 1) / steps
            eased = 1 - (1 - t) ** 3
